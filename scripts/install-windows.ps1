param(
    [switch]$Build,
    [string]$Destination = (Join-Path $env:ProgramFiles "hiyoko-viewer")
)
...
function Test-HiyokoViewerInstallDirectoryName {
    param(
        [Parameter(Mandatory = $true)]
        [string]$Path
    )
    ...
    return (
        $leafName -eq "hiyoko-viewer" -or
        $leafName.StartsWith("hiyoko-viewer-", [StringComparison]::OrdinalIgnoreCase)
    )
}
...
function Test-OnedirInstall {
    ...
    return (
        (Test-Path -LiteralPath (Join-Path $Path "hiyoko-viewer.exe") -PathType Leaf) -and
...
function Test-HiyokoViewerProcessInDirectory {
    param(
        [Parameter(Mandatory = $true)]
        [string]$Path
    )
    ...
    foreach ($process in Get-Process -Name "hiyoko-viewer" -ErrorAction SilentlyContinue) {
        try {
...
$Source = Join-Path $RepoRoot "dist\hiyoko-viewer"
...
if (-not (Test-HiyokoViewerInstallDirectoryName -Path $DestinationFullPath)) {
    throw "Destination directory name must be hiyoko-viewer or hiyoko-viewer-*: $DestinationFullPath"
}
...
if ($Build) {
    ...
    try {
        Write-Host "Building hiyoko-viewer..."
        cmd /c make_build.bat
...
try {
    if (-not (Test-Path -LiteralPath $Source -PathType Container)) {
        throw "Build output was not found: $Source. Run make_build.bat first, or pass -Build."
    }
    if (-not (Test-Path -LiteralPath (Join-Path $Source "hiyoko-viewer.exe") -PathType Leaf)) {
        throw "Build output was not found: $Source. Run make_build.bat first, or pass -Build."
    }
    ...
    if (Test-HiyokoViewerProcessInDirectory -Path $DestinationFullPath) {
        throw "hiyoko-viewer is running from $DestinationFullPath. Close it before installing."
    }
    ...
    try {
        ...
        if ($hasExistingInstall) {
            if ((-not (Test-OnedirInstall -Path $DestinationFullPath)) -and
                ...
                throw "Destination exists but does not look like a hiyoko-viewer onedir install: $DestinationFullPath"
            }
        ...
        Write-Host "Installed hiyoko-viewer to $DestinationFullPath"
    } finally {
        Remove-Item -LiteralPath $stagingDestination -Recurse -Force -ErrorAction SilentlyContinue
